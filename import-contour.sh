...
function import_shp() {
  ...
  shp2pgsql $a_parameter -s 4326:3857 -I -g geometry "$shp_file" "$table_name" | exec_psql | hide_inserts
}
...
function generalize_contour() {
  ...
  local gen_command="CREATE TABLE $gen_table AS SELECT id, elevation, ST_SimplifyPreserveTopology(geometry,$tolerance) as geometry from $table;"
...
function create_empty_table() {
  ...
  local create_command="CREATE TABLE $table(id integer, elevation double precision, geometry geometry);"
...
function import_contour() {
  ...
  if [ "$(ls -A $IMPORT_CONTOUR_DIR/*.zip 2> /dev/null)" ]; then
    ...
    echo "No ZIP files found in contour folder $IMPORT_CONTOUR_DIR."
